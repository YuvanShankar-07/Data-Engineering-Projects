...
-- MAGIC
-- MAGIC pattern3_df = spark.table("yuvan_05_may.pattern3")
-- MAGIC
-- MAGIC record_count = pattern3_df.count()
-- MAGIC
-- MAGIC if record_count >0 :
-- MAGIC     Detection_time = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
-- MAGIC     print("Records Found for PATTERN 3 " + str(record_count))
...
-- MAGIC
-- MAGIC pattern3_detection_df = Chunk_dataframe.alias("chunks").join(
-- MAGIC     pattern3_df.alias("p1"),
-- MAGIC     on=[
-- MAGIC         F.col("chunks.merchant") == F.col("p1.merchant")
-- MAGIC     ],
-- MAGIC     how="inner"
-- MAGIC )
-- MAGIC
-- MAGIC pattern3_detection_df.show(1)
-- MAGIC
-- MAGIC if pattern3_detection_df.count() == 0 :
-- MAGIC     print("No Records Found For Detection")
-- MAGIC     dbutils.notebook.exit("Stopped early due to No Records Found.")
-- MAGIC
-- MAGIC pattern3_add_colums = pattern3_detection_df.withColumn("YStartTime", F.lit(JOBstarttime)) \
-- MAGIC                     .withColumn("detectionTime", F.lit(Detection_time)) \
-- MAGIC                     .withColumn("patternId", F.lit("PatId3")) \
-- MAGIC                     .withColumn("ActionType", F.lit("DEI-NEEDED"))
-- MAGIC
-- MAGIC pattern3_detections_final = pattern3_add_colums.select(
-- MAGIC     F.col("YStartTime").alias("YStartTimeIST"),
-- MAGIC     F.col("detectionTime").alias("detectionTimeIST"),
-- MAGIC     "patternId",
-- MAGIC     "ActionType",
-- MAGIC     F.lit("").alias("CustomerName"),
-- MAGIC     F.col("chunks.merchant").alias("MerchantId")
-- MAGIC )
-- MAGIC
-- MAGIC processed_dataframe = pattern3_detections_final
-- MAGIC
...
-- MAGIC
-- MAGIC print("Detection found for Pattern 3 " + str(processed_dataframe.count()))
-- MAGIC
