...
-- MAGIC
-- MAGIC pattern1_df = spark.table("yuvan_05_may.pattern1")
-- MAGIC
-- MAGIC record_count = pattern1_df.count()
-- MAGIC
-- MAGIC if record_count >0 :
-- MAGIC     Detection_time = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
-- MAGIC     print("Records Found for PATTERN 1 " + str(record_count))
...
-- MAGIC
-- MAGIC pattern1_detection_df = Chunk_dataframe.alias("chunks").join(
-- MAGIC     pattern1_df.alias("p1"),
-- MAGIC     on=[
...
-- MAGIC )
-- MAGIC
-- MAGIC pattern1_detection_df.show(1)
-- MAGIC
-- MAGIC if pattern1_detection_df.count() == 0 :
-- MAGIC     print("No Records Found For Detection")
-- MAGIC     dbutils.notebook.exit("Stopped early due to No Records Found.")
-- MAGIC
-- MAGIC pattern1_add_colums = pattern1_detection_df.withColumn("YStartTime", F.lit(JOBstarttime)) \
-- MAGIC                     .withColumn("detectionTime", F.lit(Detection_time)) \
-- MAGIC                     .withColumn("patternId", F.lit("PatId1")) \
-- MAGIC                     .withColumn("ActionType", F.lit("UPGRADE"))
-- MAGIC
-- MAGIC pattern1_detections_final = pattern1_add_colums.select(
...
-- MAGIC )
-- MAGIC
-- MAGIC processed_dataframe = pattern1_detections_final
-- MAGIC
...
-- MAGIC
-- MAGIC print("Detection found for Pattern 1 " + str(processed_dataframe.count()))
-- MAGIC